<script>
  ...
  let selectedDates = { start: null, end: null }; // <- object, not array
  ...
  function handleDatesSelect(event) {
    selectedDates = event.detail || { start: null, end: null };
    showBookingForm = !!selectedDates.start;
  }
  ...
  function handleBookingSubmit(event) {
    ...
    setTimeout(() => {
      if (selectedFacility && selectedFacility.requiresApproval === false) {
        toastMessage = 'Booking confirmed! Your reservation is ready.';
        toastType = 'success';
      } else {
...
<section class="relative min-h-screen flex items-center justify-center overflow-hidden">
  ...
  <div class="relative z-10 text-center px-4 max-w-4xl mx-auto">
    ...
    <div class="animate-slide-up" style="animation-delay: 0.9s;">
      <button class="btn-primary text-lg px-8 py-4 pulse-glow" on:click={scrollToBooking}>
        🚀 Start Booking
      </button>
    </div>
  </div>
...
{#if showToast}
  <Toast type={toastType} message={toastMessage} on:close={() => (showToast = false)} />
{/if}
